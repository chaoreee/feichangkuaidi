...
- 最后更新：2026-07-03
- 当前轮次：Iteration 18（M8 博弈投影层 **P3 §6.2 任务 race**（默认关）：`decision.py` 追平(`ENABLE_TASK_RACE`：对手≥80 且我方<90 放宽任务绕路目标/上限，`_task_catch_up_active`)+Deny(`ENABLE_TASK_DENY`：`_task_deny_target` 用 ETA 抢占对手正奔赴、可更早到达、跨其里程碑的任务点)。两者过 `_can_afford`+ΔEV/净分守卫。14 项新单测(共 202)全通过；两开关默认关故 mock 零回归 @r48）
- 上一轮次：Iteration 17（M8 P3 §6.1 对手轨迹 ETA 纯观测：`OpponentEta`+`build_opponent_eta`；8 项新单测(共 188)）。更早各轮见 §7 迭代日志。
- 进度：**P2 全部接入**（§5.1 行1-5、§5.2-§5.4）；**P3 §6.1 ETA 观测 + §6.2 任务 race（默认关）已就绪**。所有增量动作均过 `_can_afford`+（相关处）ΔEV 地板，信息不足默认 EVEN=既有基线。下一步 P0 真实 trace 校准 或 §6.3 鲜度/资源 race。
...
| P3-P4 其余（§6.3 鲜度·资源 race/§7 条件化 SET_GUARD） | ❌ | 待实现；`ENABLE_RESOURCE_DENY`/`ENABLE_CONDITIONAL_GUARD` 已登记默认关；逐项过 `_can_afford`+ΔEV 地板、真实 trace 验证为正后打开（见 `docs/game_theory_projection_strategy.md` §6.3-§7、P3-P4） |

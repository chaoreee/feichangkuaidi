...
- 最后更新：2026-07-04
- 当前轮次：Iteration 28（**Phase B v2 + ΔEV 每帧效率门 — 机制验证成功（消除 v2 回归）但 samples 上仍中性，flag 保持关**）。落地 `docs/calibration_v1.md` §7「下一步①」：`plan_route` 改道门由纯绝对增益门升级为「绝对增益门 **与** 每帧效率门」——`gain/extra_frames ≥ STATIC_PLANNER_MIN_ROUTE_EFFICIENCY(0.2)`，且效率门**仅对长绕路**（extra≥15 帧）生效（短绕路时间成本估计可信，仅绝对门把关）。`_best_score_for_path` 扩返回 deliver_frame 供门控计算真实总时间成本。阈值 0.2 校准依据：v2 实际 −3.7 vs 投影 +7 / 60 帧 ≈ 0.18/帧乐观修正率（即修正后盈亏平衡 ratio）——拒 0.12（v2 鲜度长绕路）、纳 0.26（任务长绕路）。**验收**：268 client 单测全过（新增 3 项效率门测试：低效长绕路被拒/高效长绕路被纳/短绕路跳过效率门）；sim A/B 50 种子 **机制验证成功但未过门槛**——flag-on mean 747.9 vs baseline 747.8（**+0.1，CI[−1.8,+1.9] 中性**）、交付帧 +60→+5.5、task 140/140 无回归、分段全一致、0 STUCK、对账 0 误差。效率门消除了 v2 双重回归（−3.7→+0.1、+60→+5.5 帧），但 samples 图结构上无廉价鲜度→中性非正向。`ENABLE_STATIC_PLANNER` **保持默认关**（CLIENT_VERSION 不 bump），代码保留作通用 variant 平台。**不扫阈值逼正向**（乐观修正率 0.18 是物理下界，降阈值重演 v2、升阈值更中性，samples 无正向交叉点，扫只会过拟合初赛图）。详见 `docs/calibration_v1.md` §8。
- 上一轮次：Iteration 27（Phase B v2 联合规划器，A/B −3.7 未过门槛）。更早各轮见 §7 迭代日志。
- **〔Iter 29+ 规划〕打败前十名 P0–P3 蓝图✅定稿**（`docs/beat_top10_design.md` 详细设计与实现 / `docs/beat_top10_plan.md` 总览）。新证据：10 局对平台前十名真实报告（`reports/`，clientVersion=iter25，N=10 假设级）**4W/6L（40%）**，我方仍是固定点（score 755、deliverFrame 444、freshness 79.9、goodFruit 97、task 150、1 冰鉴）——胜负 100% 由对手鲜度决定（oppFr<80 我 4/4 全胜、oppFr≥82 我 0/5 全负，分隔线 ~82）；负局差距小（L1 −6 / L10 −2 / L9 −16 / L3 −21 / L6 −22）；**1 局灾难性未交付 vs2735（60 分）**——对手进攻性设卡封 S10、我方 `MOVE_BLOCKED_BY_GUARD` 连拒 224 帧未交付。根因定位：`_keep_moving`（decision.py:138-150）重发 `MOVE(next_node_id)` 不检查在途目标被设卡/冷却，MOVING 态短路永不进 `_breakthrough`（与 Iter 8 同源，Iter 8 只修"无在途目标"分支）。**P0–P3 排期**：Iter29 **P0** 修设卡卡死（`_keep_moving` 校验在途目标失效→回落 `_plan`，无条件合入，预期 +1 胜）→ Iter30 **P1-B** 精简 trace（`analysis/compact.py` 从完整 trace 派生事件驱动紧凑格式 ~6–9KB/局落 `reports/`，使我 pull 可直读、绕开"原始 trace 880KB 无法上传"瓶颈；client 零改动）→ Iter31 **P1-A** client trace 富化 + parser 抽取 + aggregator 落盘对手分项分/逐帧轨迹/设卡/资源 → Iter32 用户取新 reports（含 `*.compact.log`）→ Iter33 P1 归因选 P2/P3 分支 → Iter34 P2 鲜度质量积累 → Iter35 P3 denial → Iter36+ 校准。**当前 reports 不足以设计 P2/P3**（核心未知"对手凭什么鲜度 88–93"无法回答；分析器缺对手分项分/设卡/资源/逐帧轨迹——协议层全可见、缺口在 client 不记 + parser 不抽）。新"done"标准不变：任何策略改动须过 sim A/B 50 种子 CI 正向 + 分段不回归；阈值合入须真实 trace N≥30。策略须**通用**（读 `start` 动态决策，决赛换新图）。
...
- 进度：**Phase 0✅（两批 30 局）+ Phase A 保真度✅ + Iter 25 投影/修复✅ + Phase B 杠杆确证✅ + Phase B v1 A/B❌ + Phase B v2 联合 A/B❌ + Phase B v2+效率门 A/B 中性❌（均未合入）+ Iter 29+ 打败前十名 P0–P3 蓝图✅定稿（未实现）**。M8 P1-P4 代码全部保留但 triage：Layer1/ΔEV/ETA 保留、P2/P3 race 待仿真 A/B 后逐项开关、P4 SET_GUARD **冻结**。**下一步**（按 `docs/beat_top10_design.md` 排期）：Iter29 P0 修设卡卡死（无条件合入）→ Iter30 P1-B 精简 trace 回流通道 → Iter31 P1-A 分析器数据补全 → 取新 reports → P1 归因选 P2/P3 分支 → P2 鲜度质量积累 → P3 denial → Phase C 校准。新"done"标准：任何改动须过仿真 A/B 证据（见 `iteration_plan_v2.md` §1.2）。策略须**通用**（读 `start` 动态决策，不针对初赛 samples 图适配，决赛换新图）。
...
- **〔Iteration 21 三问分析认定，最高优先〕验证真空 + 静态层未求解 + 博弈层优先级错置**：① 20 轮/M8 P1-P4 全建在 mock@r48 之上，**Iter 24 前**仓库 `logs/` **零真实对局 trace**；**Iter 24 已破局**——11 局真实平台报告到货（`reports/`，N=11 假设级），坐实 mock@r48 不可信（真实交付 444-492 mean 456），仿真器据此校准（TASK_90_REACH 0.04→1.00、交付帧 mean→455.4）。但 N=11<30，阈值/开关仍不可据此合入，`ENABLE_*` 全默认关、mode 在镜像自博弈里恒 EVEN；② 静态层 `_plan` 是贪心瀑布而非优化求解——**真实证据重定向**：task-90 在真实环境必达（`TASK_90_REACH=1.00`），原"冲 90"杠杆证伪；真实杠杆是"早交付(444-448) vs 延时换质量"的静态权衡（3 局输局皆败于此，见 `docs/p0_attribution.md`）；③ SET_GUARD ROI 最低却占 P4，GATE 验核/deny 收益最高却最弱——**GATE race 已获真实证据**（`VERIFY_GATE OBJECT_BUSY` 连拒），D1 提前。**处置见 `docs/iteration_plan_v2.md` + `docs/p0_attribution.md` §6：分析器驱动证据型迭代，Phase 0✅→A✅→B(重定义)→C→D，SET_GUARD 冻结。** 阈值/开关合入仍须 N≥30 + 仿真 A/B。
...
**M9 分析器驱动证据型迭代（Iteration 21 起，进行中，详见 `docs/iteration_plan_v2.md`）**：针对 Iteration 21 三问认定（验证真空 / 静态层未求解 / 博弈层优先级错置），把迭代方式改为证据型闭环 `假设→仿真/trace 证据→实现→A/B→仅当正向才固化`。落地顺序：**Iter 21 分析器基础设施**（in-client `collector.py` + `report.json` schema + repo `analyze_logs.py` + 单测+对账，只抽取不优化）→ **Phase 0** 真实 trace 收割 + P0 归因 → **Phase A** 高保真自博弈仿真器（物理复用 `core/rules.py`，产出同格式 report）→ **Phase B** 静态规划器（**Iter 24 真实 trace 证伪"冲 90"主杠杆后重定义为"交付时机 vs 质量积累"权衡**；鲜度投影升级已由 **Iter 25** 完成，替换 `_plan` 贪心瀑布待对手分项 trace 确认杠杆后实施）→ **Phase C** 仿真驱动阈值/开关校准（产出 `calibration_v1.md`）→ **Phase D** 博弈层重排（D1 GATE 验核 race、D2 窗口对手出牌预测、D3 deny 按 |gap| 条件化+相对分 ΔEV；D4 SET_GUARD 冻结）。新"done"标准见 `iteration_plan_v2.md` §1.2。

**M10 打败前十名（Iteration 29 起，规划定稿，详见 `docs/beat_top10_design.md`）**：基于 10 局对前十名真实报告（4W/6L、我方固定点 755、胜负由对手鲜度决定、1 局被设卡卡死未交付），按 P0–P3 推进：**P0** 修设卡卡死 bug（`_keep_moving` 在途目标失效回落 `_plan`，无条件合入）→ **P1-B** 精简 trace 派生（`analysis/compact.py`，落 `reports/` 使 trace 可回流）→ **P1-A** 分析器数据补全（client 富化 + parser 抽对手分项/设卡/资源/轨迹）→ **P2** 鲜度质量积累（抬地板 755→770，P1 归因驱动选分支）→ **P3** denial（设卡/破卡/争抢，真实 trace A/B）。铁律不变：flag 默认关、阈值合入须 N≥30、sim A/B CI 正向 + 分段不回归。

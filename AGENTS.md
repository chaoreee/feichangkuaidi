...
- 最后更新：2026-07-03
- 当前轮次：Iteration 11（M8 博弈投影层 P1+P1.5 落地：`strategy/projection.py` 投影总线（纯观测，不改动作）+ 风险档位状态机 + 分数质量地板 `net_score_delta`；`strategy/tuning.py` 档位参数映射；main 每帧 `Projection`/切档 `ModeChange` trace；config 增 §9 常量；30 项新单测(共 126)全通过；mock 端到端仍 @r48 交付、行为逐帧一致）
...
| 档位参数映射（Layer 2 基础设施） | ✅ | `strategy/tuning.py` `tuning_for_mode`：mode→{task_seek_target/detour/ΔEV 阈值}；EVEN 严格等于既有默认；三档阈值均非负。**尚未被决策消费**（P2 起接入，需真实 trace 校准） |
| 对手投影驱动的风险档位调节动作（Layer 2-4） | ❌ | 档位调参/悬赏机会主义/终局 race/窗口 EV/任务·资源 race/条件化 SET_GUARD 均待实现；须逐项过 `_can_afford`+ΔEV 地板、默认关、真实 trace 验证为正后逐项打开（见 `docs/game_theory_projection_strategy.md` §5-§7、P2-P4） |
...
- `world.opponent` 已进入投影总线（Layer 1，`strategy/projection.py`）：每帧投影双方终局分/交付帧→gap→mode，但**当前仅纯观测**（只写 `Projection`/`ModeChange` trace，不改任何动作）。基于 gap 的档位调参与博弈动作（Layer 2-4）尚未接入决策，须真实 trace 校准 `LEAD_SAFE`/confidence 后逐项开启。
...
**M8 博弈投影层（P1+P1.5 已实现，P2-P4 待真实 trace 校准）**：把 `world.opponent` 升级为策略一等输入，用"对手投影驱动的风险档位切换 + 分数质量地板"提升胜率与得分上限。落地顺序（详见 `docs/game_theory_projection_strategy.md` §10）：P0 真实 trace 败局归因（待真实 `logs/`）→ **✅ P1 投影总线**（`strategy/projection.py`，纯观测、零风险、不改动作）→ **✅ P1.5 分数质量地板** `net_score_delta` → P2 低风险增量（档位/悬赏/终局 race/窗口 EV）→ P3 中风险 race（ETA/任务/鲜度/资源，逐项开关）→ P4 条件化 SET_GUARD（ROI 最低、最后做）。铁律：所有增量动作须同时过 `_can_afford`（时间）与 `ΔEV≥0`（分数）；信息不足默认 `EVEN` 保持基线。**下一步**：拿真实对局 trace 归因（P0）+ 校准 `LEAD_SAFE`/confidence/投影精度，再逐项接入 Layer 2 档位调参（`tuning.py` 已就绪）。

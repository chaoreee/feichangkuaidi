...
**〔Iter 38 追加 §3 实际结论〕** §3 真实 A/B（N_old=67 iter31 vs N_new=40 iter36）证实 **static_planner 真实对局是行为 no-op**：0/40 局走 canonical 大路（两版都走 S06→S08→S10 山路骨架）、`ICE_KEEP=3` 未生效仍领 ~1 冰鉴、S10 到达帧 267/268 相同、交付帧 409/406 相同 → §1.3"plan_route 真实图选大路"是**静态图结论**实战不成立、+20 从未激活。表观回归主因是对手池构成混杂（iter31 刷弱对手 2769 11 次全胜、iter36 仅 2 次），去偏斜后共同对手胜率/交付率差异不显著（p=0.17/0.29）。**判决：合入依据含 sim/静态投影影响且实战 no-op → Iter 38 `ENABLE_STATIC_PLANNER` 回退为关**，恢复干净基线；代码保留待"修通 plan_route 实战选路根因"后由真实 A/B 正向再开。详见 `reports/iter36_ab_out/section3_verdict.md`。

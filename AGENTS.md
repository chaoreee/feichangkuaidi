...
- 最后更新：2026-07-02
- 当前轮次：Iteration 8（M8 交付冲刺 + 任务上限，基于真实败局日志迭代）
...
| 皇榜任务取舍（机会式 + 预算内绕路，上限 90）+ 鲜度管理 | ✅ | 任务分<90 才做/绕路（90 已解锁满额送达+用时，再多不值得冒险）；预算守卫计入验核耗时 |
| 交付冲刺模式（临近终局保交付） | ✅ | `_must_commit_deliver`：剩余帧不足以"送达估算+缓冲"时放弃一切可选动作，直奔验核+交付（源于真实败局：过度做任务超时未交付） |
...
- 任务上限设为 90（`config.TASK_SEEK_TARGET`）：这是保守取舍——90 解锁满额送达基础分(240)与用时系数+35 里程碑，放弃了 110 里程碑(+15)与 90→180 的原始任务分(至多 +90，且未交付时封 80)，以换取"确保交付"。真实败局 local-debug-l1 farm 到 180 却未交付、总分仅 80。拿到更多对局数据后可上调该阈值。
...
| Iteration 8 | 2026-07-02 | 真实败局 local-debug-l1（未交付、S14/WAITING@600、任务farm到180却总分仅80）驱动 | 加交付冲刺模式(`_must_commit_deliver`临近终局直奔验核+交付)、任务上限 90(停止绕路/机会式做任务)、`_can_afford` 计入验核耗时；analyzer 增"任务达标却未交付=过度farm"建议；新增 7 项单测(共 98 client+5 analysis)全通过 | 保证交付优先，杜绝过度做任务导致超时丢局 | `logs/match_local-debug-l1_1001.analysis.md`、`CHANGELOG.md` |

...
# ---- 策略调参（M4 收益策略）----
ICE_BOX_USE_BELOW = 81.0            # 鲜度低于此且持有冰鉴时使用：在跌破 80 好果转坏阈值前补鲜度（Iter 34，见下注）
# Iter 34：78→81。冰鉴 +10（任务书 §3.3.1，好果转坏不可逆）。原 78 在跌破 80 之后才用，
# 80 阈值已转坏、冰鉴只能护 70——而真实路线交付鲜度 ~83（≥80），70 本不会触底，冰鉴被浪费。
# 提到 81：在跌破 80 前用（80.x→90.x），交付鲜度不变（=100−loss+10，与时机无关）但 MIN 鲜度
# 从 ~77 抬到 ~81，80 阈值不再触发 → 救回 1 篓好果（+1.8 分）。数学上弱优于（交付鲜度<80 的
# 长损耗路线两者坏果数相同；交付鲜度≥80 的路线 81 严格省 1）。详见 docs/iter34_route_lever_analysis.md。

...
- 最后更新：2026-07-03
- 当前轮次：Iteration 10（博弈投影层设计定稿：`docs/game_theory_projection_strategy.md` 经代码/协议核对后优化——修正 mock 交付帧误判、新增分数质量地板 ΔEV、明确 P1 纯观测先行；同步 AGENTS 能力矩阵/Roadmap/已知限制）
...
| 对手投影驱动的风险档位切换（博弈层） | ❌ | 设计见 `docs/game_theory_projection_strategy.md`：每帧投影双方终局分→gap→mode(保守/均衡/进取)调节风险；新增分数质量地板 `ΔEV≥0` 守分数下限。分层落地、默认关、真实 trace 校准 |
...
- `world.opponent` 已解析但策略尚未据其决策（`decision.py` 无引用）；全信息对抗（协议 §7 双方 players 全字段可见）为"对手投影驱动"的博弈层提供了数据基础，设计见 `docs/game_theory_projection_strategy.md`（尚未实现）。

## 6. 后续规划（Roadmap）

M0 文档基线（本轮，✅ 交付中）→ M1 通信打通 → M2 核心镜像 → M3 基线策略（稳定交付）→ M4 收益策略 → M5 对抗策略 → M6 分析闭环 → M7+ 按真实日志迭代。里程碑详情见 `docs/architecture.md` §Roadmap。

**M8 博弈投影层（设计已定，待实现）**：把 `world.opponent` 升级为策略一等输入，用"对手投影驱动的风险档位切换 + 分数质量地板"提升胜率与得分上限。落地顺序（详见 `docs/game_theory_projection_strategy.md` §10）：P0 真实 trace 败局归因 → P1 投影总线（纯观测、零风险、不改动作）→ P1.5 分数质量地板 `net_score_delta` → P2 低风险增量（档位/悬赏/终局 race/窗口 EV）→ P3 中风险 race（ETA/任务/鲜度/资源，逐项开关）→ P4 条件化 SET_GUARD（ROI 最低、最后做）。铁律：所有增量动作须同时过 `_can_afford`（时间）与 `ΔEV≥0`（分数）；信息不足默认 `EVEN` 保持基线。

...
- 最后更新：2026-07-03
- 当前轮次：Iteration 17（M8 博弈投影层 **P3 §6.1 对手轨迹 ETA**（纯观测）：`projection.py` `OpponentEta`+`Projector.build_opponent_eta` 估算对手到宫门/终点/任务·资源点帧数（在途按 move_progress 加残余帧、未验核计验核耗时；置信随终局升、轨迹变化打折）；`decision.py` 每帧构建存 `opponent_eta`、**不改动作**；main 每帧 `Eta` trace。8 项新单测(共 188)全通过；mock 零回归 @r48）
- 上一轮次：Iteration 16（M8 **P2 全部完成**：§5.1 行3 突破烧好果意愿——`_prefer_forced_pass`；10 项新单测(共 180)）。更早各轮见 §7 迭代日志。
- 进度：**P2 低风险增量全部接入**（§5.1 行1-5、§5.2、§5.3、§5.4）；**P3 §6.1 ETA 观测层已就绪**（为 §6.2/§6.3 race 提供 tie-breaker 输入）。所有增量动作均过 `_can_afford`+（相关处）ΔEV 地板，信息不足默认 EVEN=既有基线。下一步 P0 真实 trace 校准 或 P3 动作层（默认关+逐项开关）。
...
| P3-P4 动作层（§6.2 任务 race/§6.3 鲜度·资源 race/§7 条件化 SET_GUARD） | ❌ | 待实现；子能力开关 `ENABLE_TASK_DENY`/`ENABLE_RESOURCE_DENY`/`ENABLE_CONDITIONAL_GUARD` 已登记默认关；逐项过 `_can_afford`+ΔEV 地板、真实 trace 验证为正后打开（见 `docs/game_theory_projection_strategy.md` §6.2-§7、P3-P4） |

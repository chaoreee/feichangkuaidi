...
**结论**：§2 stage = 开 `ENABLE_STATIC_PLANNER`，既改路线（→大路）又改冰鉴领取（→2 冰鉴），
二者协同兑现 +20。这是方案 B 的落地路径。
...
| 阶段 | 判决 | 理由 |
|---|---|---|
| §2 stage | 🟢 开 `ENABLE_STATIC_PLANNER` | §1.3 证 plan_route 选大路；§1 证 +20。须过 sim 50 种子回归门（0 STUCK / 0 对账 / 分段不回归 / mean 不降） |
| §3 真实 A/B | 必做 | sim 是 samples 图镜像自博弈，大路收益可能不重现（对手也走大路→对称）；真实 A/B N≥30 是合入门。同时验证 Iter 33+34+36 |
| 风险 | 时间预算 | 大路多 30 帧；若对手设卡/天气恶化可能无法按时交付。`_can_afford` 时间守卫须兜底回落山路 |

**§1 完成，不合入策略改动**。§2 stage（开 static_planner + sim 回归门）+ §3 真实 A/B 留待用户/codeagent。

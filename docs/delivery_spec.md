...
- [x] 日志格式：人类可读 trace，每行一事件 `<时钟> <Event> matchId=..., round=..., k=v`，写入 **`client/logs/`**（包内），供取回后由仓库侧 `analysis/` 模块解析归因（见下"日志事件类型"）。日志增厚为分析服务，不违反 §10.7 自检（仍逐行 flush、单行一事件、不驻留内存）

日志事件类型（`client/logger/match_logger.py` + `client/main.py` 构造）：
- `Startup / Register / Ready / Shutdown` — 生命周期
- `Start matchId, teamId, camp, durationRound, nodes, edges, gate, terminals, processNodes` — 开局地图角色（供分析重建骨架）
- `Frame round, phase, node, state, fresh, goodFruit, taskScore, verified, delivered, weather?, opp=?, events` — 每帧本方状态 + 天气 + 对手镜像（`opp=node|state|fresh|goodFruit|taskScore|verified|delivered`，缺段写 `-`）+ 事件类型
- `Action round, action, target/task/resource/contest/card/rush/good/bad/extraGood, ms?` — 每个动作（超预算附 ms）
- `Block round, node, obstacle?, guardOwner?, guardDef?, cleared?` — 阻塞快照按变化触发（新增/变化/解除）
- `Contest round, contestId, type, ri, myPt, oppPt, myCard, oppCard` — 本方参与窗口每拍
- `Reject round, action, target, code` — 本方被拒动作
- `Budget round, est, left` — 交付估值（分析预算漂移）

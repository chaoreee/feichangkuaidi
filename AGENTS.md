...
| 资源领取与使用（冰鉴保鲜/马加速） | ✅ | 机会式领取+移动中用马；情报(INTEL)待用 |
| 皇榜任务取舍（机会式，不绕路）+ 鲜度管理 | ✅ | 目标即当前节点才做，时间预算守卫保证仍能交付；冰鉴护阈值 |
| 阻塞感知路由（障碍/敌卡绕行） | ✅ | `time_optimal_path(blocked=...)`；仿真中障碍不可绕行时突破 |
| 突破：障碍→T04/CLEAR/强制通行 | ✅ | `_breakthrough`；仿真 CLEAR 清障后继续；保留最低好果 |
...
| 主动设卡/进攻干扰、小分队清障/增援/削弱 | ❌ | action 能力已具备，基线不主动使用（占用己方交付时间/时序复杂），留待专门进攻策略 |
| 绕路做任务 / 情报探路减时 / 绕行-清障代价权衡 | ❌ | 后续优化 |
...
- M3 基线策略仅按"到站移动量"选最短路，**未计入固定处理耗时/资源/任务收益**（例：仿真中选了含 S04/S05 两处处理的水路而非官道）；也**未处理设卡/障碍/窗口**——遇阻会重复 MOVE 被拒。这些由 M4（收益路由）与 M5（对抗）解决。
- 移动被拒（PROCESS_REQUIRED 以外）暂无回退：M3 依赖清洁路线；真实对抗需 M5 补齐。

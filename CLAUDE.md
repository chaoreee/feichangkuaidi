...
- 最后更新：2026-07-04
- 当前轮次：Iteration 23（**Phase A 高保真自博弈仿真器**，in-process，物理复用 `core/rules.py`）。新建 `scripts/sim_engine.py`（忠实物理：移动按路线距离×耗时系数、鲜度逐帧损耗+阈值转坏、天气 4 次×60 帧、探路标记 45 帧+处理/验核 -3、RUSH 四条件+450 强制、**路线边空动作 park WAITING**＝Iter8 行为）+ `scripts/sim_validator.py`（`rules.py` 重算对账 over_data 0 误差）+ `scripts/sim_server.py`（双 `DecisionEngine` 自博弈，复用 `main._log_*` 写 trace 到 `logs/sim/<variant>/match_<seed>_<pid>.log`，matchId=`sim_<variant>_s<seed>` 供聚合器按 seed 配对 A/B；CLI `python3 -m scripts.sim_server --games 50 --seeds 1..50 --variant baseline`）。trace 与真实客户端同格式、`analysis/parser.py` 可读。**验收**：50 局交付率 1.000、交付帧 428–459（mean 436.7，落在 [400,520]、非 ~r48）、SimValidator 0 误差、0 卡死；`python3 -m analysis logs/sim/baseline/` 产 `reports/`（分段视图齐全、对账 ok=100/mismatch=0、`TASK_90_REACH=0.04` 已暴露 Phase B 杠杆）。18 项 sim 单测 + client 231 + analysis 42 全过。**待真实 trace 校准**：悬赏/窗口留空、资源不动态刷新、任务池 seed 合成、天气区域按路线类型近似；自博弈同配置镜像→任务首到者得（RED 解析序偏置）。
- 上一轮次：Iteration 22（日志/分析架构重构：trace 完整性 + 单局报告落盘 + 时序还原；确立「client trace=传输格式 / repo 产物=分析格式」分离）。更早各轮见 §7 迭代日志。
- 进度：**Phase A 仿真器已落地**（证据型迭代的实验台就绪）。M8 P1-P4 代码全部保留但 triage：Layer1/ΔEV/ETA 保留（ΔEV 输入待 Phase B 鲜度模型升级后才可信）、P2/P3 race 待仿真 A/B 后逐项开关、P4 SET_GUARD **冻结**。**下一步**：① Phase 0（用户并行去取）真实 trace 到 `logs/real/` → `python3 -m analysis` → P0 归因 + 校准 sim 保真度；② Phase B 静态规划器（task-90 可达性+路线评分，sim 已暴露 `TASK_90_REACH=0.04` 杠杆）+ 鲜度投影升级；③ Phase C 仿真驱动阈值/开关校准（产出 `calibration_v1.md`）。新"done"标准：任何改动须过仿真 A/B 证据（见 `iteration_plan_v2.md` §1.2）。
...
| 高保真自博弈仿真器（Phase A） | ✅ | **Iteration 23 落地**：`scripts/sim_engine.py` 物理一律调 `core/rules.py`（移动按路线距离×耗时系数、鲜度逐帧损耗+阈值转坏、天气 4 次×60 帧、探路标记 45 帧+处理/验核 -3、RUSH 四条件+450 强制、**路线边空动作 park WAITING**=Iter8 行为、设卡/攻坚/强制通行公式）；`scripts/sim_server.py` 进程内双 `DecisionEngine` 自博弈，复用 `main._log_*` 写 trace 到 `logs/sim/<variant>/match_<seed>_<pid>.log`（matchId=`sim_<variant>_s<seed>` 供聚合器按 seed 配对），CLI `python3 -m scripts.sim_server --games 50 --seeds 1..50 --variant baseline`；`scripts/sim_validator.py` 用 `rules.py` 重算对账 over_data 0 误差。trace 与真实客户端同格式、parser 可读。**验收**：50 局交付率 1.000、交付帧 428–459（[400,520]、非 ~r48）、0 卡死、对账 0 误差；`analysis` 产 reports 分段视图齐全、`TASK_90_REACH=0.04`。18 项 sim 单测。**待真实 trace 校准**：悬赏/窗口留空、资源不动态刷新、任务池 seed 合成、天气区域按路线类型近似；自博弈同配置镜像→RED 解析序偏置 |

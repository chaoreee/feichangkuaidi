...
- 进度：**Phase 0✅（两批 30 局）+ Phase A 保真度✅ + Iter 25 投影/修复✅ + Phase B 杠杆确证✅ + Phase B v1 A/B❌ + Phase B v2 联合 A/B❌ + Phase B v2+效率门 A/B 中性❌（均未合入）**。M8 P1-P4 代码全部保留但 triage：Layer1/ΔEV/ETA 保留、P2/P3 race 待仿真 A/B 后逐项开关、P4 SET_GUARD **冻结**。**下一步**（按 `docs/calibration_v1.md` §8）：① 接受 samples 中性、靠决赛新图（冰源与任务点共址、绕路短 ratio 高时）自然正向——效率门纳 0.7/帧高效绕路而 v2 误纳低效者；② 继续收割至 N≥30 脱"假设级"；③ D1 GATE race（保持 Phase B 后首位，真实 trace 已有 `OBJECT_BUSY` 证据）；④ Phase C 阈值/开关校准。新"done"标准：任何改动须过仿真 A/B 证据（见 `iteration_plan_v2.md` §1.2）。策略须**通用**（读 `start` 动态决策，不针对初赛 samples 图适配，决赛换新图）。

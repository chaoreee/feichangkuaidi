...
- 最后更新：2026-07-05
- 当前轮次：Iteration 33（**真实对战基线 P0.2 — 修复 MOVING_ACTION_FORBIDDEN 重试风暴致 3 局未交付，无条件合入**）。Iter 32.5 codeagent 跑出 N=67 真实对战基线（`reports/`，clientVersion=iter31，胜率 64%、交付率 96%）。群体归因定位 **3 局未交付（4%）** 为最大失分点（每局 me≈67 必负）；三局 compact trace 同根因：到达对手设卡关隘 S10 后进入 `WAITING` 态，`_keep_moving` 回落 `_plan` 连发非 MOVE 节点动作，全被 `MOVING_ACTION_FORBIDDEN` 拒、连发成风暴烧光交付窗口（`032341` BREAK_GUARD×52+ICE_BOX×287、`035502` BREAK_GUARD×54+ICE_BOX×201、`034523` FORCED_PASS×60+RUSH_PROTECT×180）。与 Iter 25（CLAIM_TASK 风暴）/Iter 29（设卡死锁）同源——拒绝反馈未消费成"不重发"。**修复**：泛化 Iter 25/29 拒绝冷却模式——`_apply_rejection_feedback` 对**非 MOVE 动作 + MOVING_ACTION_FORBIDDEN** 按签名 `(action,target,resource)` 冷却（`REJECT_ACTION_COOLDOWN_ROUNDS=8`，USE_RESOURCE 区分 ICE_BOX/HORSE 不误伤）；`_freshness_rescue`/`_maybe_rush_protect`/`_maybe_bounty`/`_breakthrough` 4 个风暴发起点加 `_action_cooled` 门——冷却中跳过 → `_plan` 落到 `_advance`(MOVE) 重路由绕行；`_window_card` MOVING/WAITING 态只出 ABSTAIN。`DELIVER`/`VERIFY_GATE` 不受该码影响（不冷却、不门控）。纯 bug 修复，零策略风险，不开 flag 不涉阈值；`CLIENT_VERSION` iter31→iter33。**验收**：391 单测全过（283 client[273+10 新 `test_action_cooldown.py`] + 90 analysis + 18 sim）；sim 50 种子回归门（1.000 交付/0 STUCK/对账 0 误差/帧 455.4 分 747.8 零回归）。预期转化 3 局未交付→交付，抬地板 729→~760。详见 `CHANGELOG.md` §[33]。
- 上一轮次：Iteration 32（codeagent 真实对战基线准备 — 对手策略分类器 + 群体归因段，纯观测）。更早各轮见 §7 迭代日志。
...
- 进度：**Phase 0✅（两批 30 局）+ Phase A 保真度✅ + Iter 25 投影/修复✅ + Phase B 杠杆确证✅ + Phase B v1/v2/v2+效率门 A/B 均未合入 + beat_top10 P0✅（Iter 29，设卡卡死修复无条件合入）+ P1-B✅（Iter 30，精简 trace 派生，纯观测）+ P1-A✅（Iter 31，分析器数据补全，纯观测）+ Iter 32✅（对手策略分类器 + 群体归因段，纯观测）+ Iter 32.5✅（codeagent 跑出 N=67 真实基线，64% 胜率/96% 交付）+ Iter 33✅（P0.2 MOVING_ACTION_FORBIDDEN 重试风暴修复，3 局未交付根因，无条件合入）+ P2/P3 规划定稿未实现**。M8 P1-P4 代码全部保留作**工具箱**（race/guard flags 默认关），由 beat_top10 排期驱动逐项评估，不再单列。**前进计划严格按 `docs/iteration_loop_design.md`**（旧 M9 Phase D / calibration_v1 §8 已取代，见 §6）：~~Iter29 **P0** 修设卡卡死~~✅ → ~~Iter30 **P1-B** 精简 trace 回流通道~~✅ → ~~Iter31 **P1-A** 分析器数据补全~~✅ → ~~Iter32 **群体归因段** 对手分类器 + 对手类分桶~~✅ → ~~Iter32.5 codeagent 跑首轮真实基线（N=67）~~✅ → ~~Iter33 **P0.2** MOVING_ACTION_FORBIDDEN 重试风暴修复~~✅ → **Iter34+ 静态最优**（quality-route 桶鲜度杠杆：抬地板 729→760+，Phase B 静态规划器 codeagent 真实 A/B N≥30）→ **Iter35+ 博弈最优**（对手策略分类器 + 对手类驱动策略切换）→ **Iter36+ P3 denial**。新"done"标准：**sim A/B 50 种子降为回归 + 不变量门**（0 STUCK/对账 0 误差/分段不回归；镜像自博弈无法验证博弈层）；**真实对战 A/B 升为合入门**（新 vs 老 client 对同一对手池 N≥30 正向才合入）；阈值合入须真实 trace N≥30；验证为负则删。策略须**通用**（读 `start` 动态决策，决赛换新图）。

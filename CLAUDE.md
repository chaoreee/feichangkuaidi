...
- 最后更新：2026-07-04
- 当前轮次：Iteration 22（**日志/分析架构重构：trace 完整性 + 单局报告落盘 + 时序还原**）。确立「**client trace = 传输格式（单文件、抗回传）/ repo 产物 = 分析格式（多文件、可重生成）**」分离原则：client 仍只写一个 `match_*.log`（不拆多文件，因平台回传契约只保证该文件），但补全事实缺口——新增 `Map` 事件（拓扑快照解锁路线归因）、`Frame` 加对手逐帧 `oppNode/oppFresh/oppGood/oppTask` + `weather`、`Action` 加决策时刻 `fresh/goodFruit/gap`、`Bounty` 事件（悬赏机会主义快照）。repo 侧 `python3 -m analysis` 产出**多文件**（统一落仓库根 `reports/`，与规格文档 `docs/` 解耦）：每局 `reports/match_<id>.report.json`（含 `decisionTimeline`，解决"只在内存读不到"）+ `reports/index.json`（matchId→outcome/score/luckClass/segments/reportPath）+ `reports/analysis_report.md` + `reports/ab_report.md` + `reports/timelines.md`（异常局关键事件链）。**内网边界**：`logs/**/*.log` gitignore（仅内网采集分析、不上传）；`reports/` **入库上传**——内网跑完 analysis 后 commit/push，外部 Claude Code pull 即可读分析（外部无 logs/、无需重跑）。parser 修 `weather_hit` 死字段、`decisionTimeline` 去 60 条截断、`USE_RESOURCE` 入 timeline、修 matchId 占位 `-` 恢复。42 项 analysis 单测（+10）+ 231 client 单测（共 273）。mock 端到端 @r48 交付零回归、对账 0 误差。
- 上一轮次：Iteration 21（分析器基础设施落地·修订：分析模块移出 client，事后解析 trace）。更早各轮见 §7 迭代日志。
- 进度：**日志/分析架构重构已落地**（trace 完整性 + 单局报告落盘 + 时序还原）。M8 P1-P4 代码全部保留但 triage：Layer1/ΔEV/ETA 保留（ΔEV 输入待 Phase B 鲜度模型升级后才可信）、P2/P3 race 待仿真 A/B 后逐项开关、P4 SET_GUARD **冻结**。**下一步（Iter 23→）**：Phase 0 用当前 client 打包提交收割真实对局 trace 到 `logs/real/` → `python3 -m analysis` → AI 读 `analysis_report.md` + `timelines.md` + 下钻 `reports/*.report.json` 做 P0 归因；再 Phase A 高保真仿真、Phase B 静态规划器。新"done"标准：任何改动须过仿真 A/B 证据（见 `iteration_plan_v2.md` §1.2）。

...
- 最后更新：2026-07-04
- 当前轮次：Iteration 21（**分析器基础设施落地（修订版）**——分析模块 `analysis/` 放在 **client 之外**（仓库根），client 只记 trace 日志、不含分析代码、不写结构化报告。`analysis/parser.py` 把 `match_*.log` 解析为 `Report`（schemaVersion=1），`analysis/aggregator.py` 跨局统计/场景分段/运气分类/异常局标记/seed 配对 A/B/`rules.py` 对账自检，CLI `python3 -m analysis`。client 侧仅补少量 trace 行（`Rejected`/`CanAffordBlock`/Start `seed`/WINDOW_CARD `contestType`）。32 项单测（14 parser + 18 aggregator，共 263）。mock 端到端 @r48 交付零回归、对账 0 误差。修订原因：初版误把 `analysis/` 放进交付件 `client/`——对战平台运行时无需实时分析，分析器属仓库侧工具。详见 `docs/iteration_plan_v2.md`。）
- 上一轮次：Iteration 20（M8 P4 §7 条件化 SET_GUARD 默认关；M8 P1-P4 全部落地）。更早各轮见 §7 迭代日志。
- 进度：**分析器基础设施已就绪（分析模块在 client 之外）**（详见 `docs/iteration_plan_v2.md`）。M8 P1-P4 代码全部保留但 triage：Layer1/ΔEV/ETA 保留（ΔEV 输入待 Phase B 鲜度模型升级后才可信）、P2/P3 race 待仿真 A/B 后逐项开关、P4 SET_GUARD **冻结**。**下一步（Iter 22→）**：Phase 0 用当前 client 打包提交收割真实对局 trace 到 `logs/real/` → `python3 -m analysis` → AI 读 `analysis_report.md` 做 P0 归因；再 Phase A 高保真仿真、Phase B 静态规划器。新"done"标准：任何改动须过仿真 A/B 证据（见 `iteration_plan_v2.md` §1.2）。
...
| 赛后分析 | ✅ | **Iteration 21 分析器驱动落地**（详见 `docs/iteration_plan_v2.md`）：client 只记 trace 日志（含 `Rejected`/`CanAffordBlock` 内部信号行 + Start `seed` + WINDOW_CARD `contestType`），**不含分析模块**；仓库根 `analysis/`（client 之外）`parser.parse_log` 把 `match_*.log` 解析为 `Report`（schemaVersion=1，纯 stdlib·永不抛出），`aggregator` **累积语料**（跨批次追加）+ 跨局统计 + **场景分段**（交付/未交付、task-90 达成/未达、中局领先/落后、天气/争抢、对手交付 等）+ seed 配对 A/B（95% CI + 分段回归检查 + 低样本标"假设级" N<30/100）+ **运气分类**（expected_win/unlucky_loss/lucky_win/expected_loss，以投影误差作 v1 运气信号）+ 异常局标记 + `rules.py` 对账自检（0 误差，mock 真实分已验）→ `analysis_report.md`/`ab_report.md`；CLI `python3 -m analysis <dirs>`，source/variant 按路径推断。Claude Code 读聚合报告归因，不直读 10w 字 trace。**代码只抽取事实、不做优化**；**单局只作假设来源，决策须基于全语料聚合+CI+分段不回归**（防单点过拟合，对齐 839cfc9 教训）。14 项 parser + 18 项 aggregator 单测 |
...
|---|---|---|---|---|---|
| Iteration 21 | 2026-07-04 | 策略三问分析（验证真空 / 静态层未求解 / 博弈层优先级错置）驱动迭代方式重排 → 落地分析器基础设施（**修订：分析模块移出 client**） | ①设计评审（未改运行期代码）：新增 `docs/iteration_plan_v2.md` 定新范式"分析器驱动证据型迭代"——**代码只抽取事实、AI 只做解释**（Iter 9 删旧 `analysis/` 后以正确形态回归）。Phase 路线：0 真实 trace 归因 → A 高保真仿真(复用 `core/rules.py`) → B 静态规划器(任务-90 可达性替换贪心瀑布) → C 仿真校准 → D 博弈层重排(GATE/deny/预测优先，SET_GUARD 冻结)。新"done"标准：改动须过仿真 A/B 证据。②**实现**：分析模块 `analysis/` 位于**仓库根（client 之外）**——`parser.parse_log` 把 client trace `match_*.log` 解析为 `Report`(schemaVersion=1，纯 stdlib·永不抛出)，`aggregator` 跨局统计+场景分段+运气分类+异常局标记+seed 配对 A/B[95% CI+分段回归+低样本标"假设级"]+`rules.py` 对账自检 → `analysis_report.md`/`ab_report.md`，CLI `python3 -m analysis`。client 侧仅日志：`main.py`/`decision.py` 把分析所需事实写进 trace（已有 Frame/Action/Projection/ModeChange/Over/Score；新增 `Rejected`/`CanAffordBlock` 内部信号行 + Start `seed` + WINDOW_CARD `contestType`，decision 经 `trace_events` 暴露、main 落盘），**不含分析模块、不写 report.json**。`mock_server.py` 改用 `core/rules.py` 算真实终局分（让 trace Score 行携带可信分）。新增 `analysis/tests/test_parser.py`(14)+`test_aggregator.py`(18) 共 +32（合计 263）全通过。mock 端到端 @r48 交付零回归、对账 0 误差。**修订原因**：初版误把 `analysis/` 放进交付件 `client/`——对战平台运行时无需实时分析，分析器属仓库侧工具，故移出。同步更新 CLAUDE.md(§2/§4.4/当前轮次)、architecture.md、delivery_spec.md、logs/README.md、iteration_plan_v2.md(§3.1/3.2/3.3/3.6/11)、CHANGELOG.md。M8 P1-P4 代码 triage：Layer1/ΔEV/ETA 保留、P2/P3 待 A/B、P4 冻结 | 分析器基础设施就绪（在 client 之外）：trace→Report→聚合报告闭环通；待 Iter 22 收割真实 trace 做 P0 归因 | `CHANGELOG.md`、`docs/iteration_plan_v2.md`、`analysis/parser.py`、`analysis/aggregator.py` |

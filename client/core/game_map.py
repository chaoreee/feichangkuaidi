...
class GameMap:
    ...
    def time_optimal_path(self, source, target, base_move=None, blocked=None,
                          weather_type=None, enter_cost_fn=None, freshness_weight=0.0):
        ...
        weather_type：当前生效天气类型；按每条边的路线类型套用天气通行倍率（暴雨×水路 1350、
        山雾×山路 1100，其余 1000），使选路对天气敏感（§2.3.2）。None 视为无影响天气。
        enter_cost_fn：可选回调 node_id -> 额外进入帧数（如敌方设卡时间税/障碍清障税），
        用于估算"交税通行"而非整节点屏蔽的真实代价；返回 0 表示无额外成本。
        freshness_weight：可选鲜度权重 λ>0。边权额外 += λ × 单边帧数 × (该路线鲜度损耗 − 水路损耗)，
        使"帧数相近"时偏好水路/官道而非山路/支路（§3.2.2）。差分式：水路本身不加惩罚，
        故对 _can_afford 等绝对预算的抬高极小（仅劣路线类型被轻微惩罚）。
        """
        ...
        adj = {}
        for e in self.edges:
            wmult = rules.weather_move_multiplier(e.route_type, weather_type)
            base = rules.frames_on_edge(e.distance, e.route_type, bm, wmult)
            penalty = 0.0
            if fw > 0:
                loss = rules.route_freshness_loss(e.route_type)
                diff = loss - rules.FRESHNESS_LOSS_MOVE_MIN
                if diff > 0:
                    penalty = fw * base * diff
            if e.to_node not in blocked:
                extra = enter_cost_fn(e.to_node) if enter_cost_fn else 0
                adj.setdefault(e.from_node, []).append((e.to_node, base + self._proc_cost(e.to_node) + extra + penalty))
            if e.bidirectional and e.from_node not in blocked:
                extra = enter_cost_fn(e.from_node) if enter_cost_fn else 0
                adj.setdefault(e.to_node, []).append((e.from_node, base + self._proc_cost(e.from_node) + extra + penalty))
        return pathfind.shortest_path(adj, source, target)

...
- 最后更新：2026-07-04
- 当前轮次：Iteration 25（**CLAIM_TASK 重试修复 + 鲜度投影升级**）。基于 `docs/p0_attribution.md` 两个低风险可下手点（不涉阈值/开关合入，符合 N<30 纪律）：① `decision.py` `_apply_rejection_feedback` 对 CLAIM_TASK+OBJECT_BUSY 设 task 级冷却（`_task_cooldown`，`config.REJECT_TASK_COOLDOWN_ROUNDS=6`），`_maybe_task` 跳过冷却 taskId——杜绝真实 trace S10 连停 30+ 帧重发同任务（13 waitingStuck）；② `projection.py` 新增 `freshness_loss_for_path`（逐边 `FRESHNESS_LOSS_MOVE[routeType]`×`frames_on_edge` + 处理站/验核停靠 `FRESHNESS_LOSS_BASE`×帧 ×天气系数），替换 `AVG_FRESHNESS_LOSS_PER_FRAME=0.06` 平摊；`_project_player` 与 ΔEV 调用方（`_detour_net_delta`/`_task_detour_target`/`_task_deny_target`/bounty）改路线感知——**ΔEV 地板输入首次可信**。**验收**：302 单测全过（client 242 含 11 新增/sim 18/analysis 42）；sim 50 局零回归（交付率 1.000/帧 mean 455.4/0 卡死/对账 0 误差）——投影升级在镜像自博弈不改动作（gap 恒 0→mode 恒 EVEN），CLAIM_TASK 冷却在 sim 不触发（sim 每玩家独立任务池不重发，CLAUDE 已注"sim 不复现客户端重试 bug"），二者待真实 trace 二次验证。**trace 版本戳**：`config.CLIENT_VERSION="iter25"`（每轮 bump）+ `code_version()` 拼 git 短 hash，Startup trace 记 `version`，parser 入 `Report.clientVersion`/index——解决"log 不记代码版本"使旧/新 client trace 可区分（支持"每轮即弃"工作流）。**推迟**：全量 static_planner（task 已封顶，真实杠杆待对手分项 trace 确认）。
- 上一轮次：Iteration 24（Phase 0 真实 trace 归因 + 仿真器保真度校准）。更早各轮见 §7 迭代日志。
- **Phase 0 第二批（19 局 iter25）归因✅**（`docs/p0_attribution_batch2.md`）：累计首批 11+本批 19=30 局。**确证并量化**"早交付 vs 质量积累"静态权衡——我方 play 是固定点（总分 755.7±1.81、交付帧 446.6、鲜度 80.4、好果 97、task 150、用 1 冰鉴，跨 19 局近乎常量），胜负 100% 由对手路线决定：oppFr<80 我 9/9 全胜、oppFr≥82 我 1/9 几乎全负（近完美分隔）。`rules.py` 精确重构 19 局 max 误差 0；**真实杠杆证伪"任务"（task_base≥130 双双封顶 delivery 240/task 180，多做任务零分）、确证"鲜度"（+19）+好果（+3，与鲜度耦合）、时间成本仅 -5**——质量路线投影 779 vs 当前 755（+24）。Iter 25 CLAIM_TASK 冷却验证生效（waitingStuck 13→2，~10×）。N=19<30 仍标"假设级"，不合入阈值。
- 进度：**Phase 0✅（两批 30 局）+ Phase A 保真度✅ + Iter 25 投影/修复✅ + Phase B 杠杆确证✅**（ΔEV 地板输入可信、静态权衡数学确证）。M8 P1-P4 代码全部保留但 triage：Layer1/ΔEV/ETA 保留、P2/P3 race 待仿真 A/B 后逐项开关、P4 SET_GUARD **冻结**。**下一步**（按 `docs/p0_attribution_batch2.md` §6）：① **Iter 26 Phase B**：实现 `strategy/static_planner.py`——鲜度感知路线（`freshness_loss_for_path`）+ 冰鉴策略 + `rules.py` 投影终局分选最高（预测质量路线 +24），作 variant；② 仿真 A/B 50 种子验收（mean 分正向 + 交付率/`mid_lead`/`delivered` 段不回归）+ 真实 trace 二次校准后才合入默认；③ 继续收割至 N≥30 脱"假设级"；④ D1 GATE race（保持 Phase B 后首位）；⑤ Phase C 阈值/开关校准。新"done"标准：任何改动须过仿真 A/B 证据（见 `iteration_plan_v2.md` §1.2）。

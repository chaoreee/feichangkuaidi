...
- [x] **结构化分析报告**（Iter 21 已实现，分析模块在 **client 之外**，详见 `docs/iteration_plan_v2.md`）：client 只记 trace 日志（含 `Rejected`/`CanAffordBlock` 内部信号行），**不含分析模块、不写结构化报告**；仓库根 `analysis/` `parser.parse_log` 把取回的 `client/logs/match_<id>_<pid>.log` 解析为 `Report`（schemaVersion=1，纯 stdlib·永不抛出），`aggregator` 聚合产出 `reports/analysis_report.md`（跨局统计 + 场景分段 + 运气分类 + seed 配对 A/B[95% CI + 分段回归 + 低样本标记] + 异常局标记 + `rules.py` 对账自检）+ `reports/ab_report.md`；CLI `python3 -m analysis <dirs>`（产物落仓库根 `reports/`，与规格文档 `docs/` 解耦）。**代码只抽取事实、不做优化**；Claude Code 读聚合报告归因，不直读 10w 字原始 trace；21 项 parser + 21 项 aggregator 单测全通过

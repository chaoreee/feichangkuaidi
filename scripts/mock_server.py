...
# 地图变体（--variant=<arena|sparse|large>）：arena=初赛竞技地图；sparse/large=合成线性链(距离 18/25)。
# 默认 arena；__main__ 解析 --variant= 后覆盖。
VARIANT = "arena"
...
def load_map():
    with open(_MAP, encoding="utf-8") as fh:
        base = json.load(fh)
    variant = VARIANT
    if variant in ("sparse", "large"):
        return build_linear_map(base, 18 if variant == "sparse" else 25)
    return base


def build_linear_map(base, distance):
    """在 arena 基线上合成线性链：保留全部节点角色/processNodes/资源/宫门/安全区不变，
    仅把 edges 替换为 S01→S02→...→S15 单链（全 ROAD、统一距离）。

    任务书 §2.2：路线边清单与边距离为可变项，节点角色/机制固定。线性链无捷径/支路/水路，
    验证最小拓扑下路由+突破(S13 障碍不可绕)+宫门验核+交付；distance=25 验证帧预算门控/
    RUSH 前置随距离标度缩放。OBSTACLES={"S13"} 仍在唯一通路上，迫使 CLEAR。
    """
    node_ids = [n["nodeId"] for n in base["nodes"]]
    mc = {k: v for k, v in base.items()}
    mc["edges"] = [
        {"fromNodeId": a, "toNodeId": b, "routeType": "ROAD", "distance": distance}
        for a, b in zip(node_ids[:-1], node_ids[1:])
    ]
    mc["mapName"] = base.get("mapName", "arena") + "_linear_%d" % distance
    return mc
...
if __name__ == "__main__":
    ...
    # --variant=<arena|sparse|large> 可出现在任意位置（test_map_variance 作第 4 参传入）。
    # arena=初赛竞技地图；sparse/large=合成线性链(距离 18/25)，验证换图无关性。
    VARIANT = "arena"
    for arg in sys.argv[1:]:
        if arg.startswith("--variant="):
            VARIANT = arg.split("=", 1)[1].strip() or "arena"
            break

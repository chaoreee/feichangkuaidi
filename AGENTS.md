...
- 最后更新：2026-07-03
- 当前轮次：Iteration 19（M8 博弈投影层 **P3 §6.3 鲜度/资源 race**（默认关）：`decision.py` 鲜度 race(`ENABLE_FRESHNESS_RACE`：鲜度劣势时 `_freshness_rescue` 提前用冰鉴到 `ICE_BOX_RACE_USE_BELOW`)+资源 race(`ENABLE_RESOURCE_DENY`：`_maybe_resource_race` 用 ETA 抢占对手争夺、库存有限、路线附近冰鉴)。12 项新单测(共 214)全通过；两开关默认关故 mock 零回归 @r48。**P3 race 层 §6.1/§6.2/§6.3 全部就绪，仅剩 Layer 4 §7 SET_GUARD**）
- 上一轮次：Iteration 18（M8 P3 §6.2 任务 race：追平+Deny 默认关；14 项新单测(共 202)）。更早各轮见 §7 迭代日志。
- 进度：**P2 全部接入**（§5.1 行1-5、§5.2-§5.4）；**P3 §6.1 ETA + §6.2 任务 race + §6.3 鲜度/资源 race（均默认关）已就绪**。所有增量动作均过 `_can_afford`+（相关处）ΔEV 地板，信息不足默认 EVEN=既有基线。下一步 P0 真实 trace 校准 或 P4 §7 条件化 SET_GUARD。
...
| P4 条件化 SET_GUARD（§7） | ❌ | 待实现；`ENABLE_CONDITIONAL_GUARD` 已登记默认关；ROI 最低最后做，把 denial 对胜负期望价值计入 ΔEV 地板、真实 trace 验证后打开（见 `docs/game_theory_projection_strategy.md` §7、P4） |
...
- `world.opponent` 已进入投影总线（Layer 1，`strategy/projection.py`）并驱动 **P2 档位调参**（§5.1 行1/2/4）：`decision.py` 每帧按 mode 调节任务绕路目标/上限与护果令阈值，绕路动作过 §3.3 ΔEV 地板。但 mode 前中段因 confidence<0.55 恒为 EVEN（=既有默认），故实战差异主要出现在中后段切档时——须真实 trace 校准 `LEAD_SAFE`/confidence 后才会频繁生效。§5.2 悬赏（`_maybe_bounty`）、§5.3 终局 race（`_endgame_race_state`+race-aware `_rush_speed_warranted`）、§5.4 窗口 EV（`_window_card`）、§5.1 行3 突破烧好果意愿（`_prefer_forced_pass`：CONSERVATIVE 领先时突破优先 FORCED_PASS 保好果）均已接入——**P2 全部完成**。P3 §6.1 对手轨迹 ETA 观测层（`OpponentEta`，每帧 `Eta` trace）、§6.2 任务 race（追平/Deny）、§6.3 鲜度/资源 race（`_losing_freshness_race` 提前用冰鉴、`_maybe_resource_race` 抢冰鉴）均已就绪，全部默认关（`ENABLE_TASK_RACE`/`ENABLE_TASK_DENY`/`ENABLE_FRESHNESS_RACE`/`ENABLE_RESOURCE_DENY`）；仅 Layer 4 §7 条件化 SET_GUARD 未实现。所有 race 均依赖 ETA（对手意图不可观测），须真实 trace 校准后逐项打开。窗口 EV 未做对手出牌倾向预测；ETA 假设对手沿最短路前进（意图不可观测，轨迹变化打折 confidence）；档位切换/ETA 精度均依赖前中段偏低的 confidence（差异主要在中后段），待 P0 真实 trace 校准。
...
**M8 博弈投影层（P1+P1.5+P2 档位调参已实现，P2 其余与 P3-P4 待续）**：把 `world.opponent` 升级为策略一等输入，用"对手投影驱动的风险档位切换 + 分数质量地板"提升胜率与得分上限。落地顺序（详见 `docs/game_theory_projection_strategy.md` §10）：P0 真实 trace 败局归因（待真实 `logs/`）→ **✅ P1 投影总线**（纯观测）→ **✅ P1.5 分数质量地板** `net_score_delta` → **✅ P2 低风险增量全部完成**（§5.1 行1-5 档位调参+ΔEV 地板、§5.2 悬赏机会主义、§5.3 终局交付 race、§5.4 窗口 EV 均已接入 `decision.py`）→ **🟢 P3 中风险 race 全部接入（默认关）**（✅ §6.1 对手轨迹 ETA 观测层、✅ §6.2 任务 race 追平+Deny、✅ §6.3 鲜度/资源 race）→ **P4 §7 条件化 SET_GUARD**（唯一待做，ROI 最低、最后做）。铁律：所有增量动作须同时过 `_can_afford`（时间）与 `ΔEV≥0`（分数）；信息不足默认 `EVEN` 保持基线。**下一步**：①拿真实对局 trace 归因（P0）+ 校准 `LEAD_SAFE`/confidence/投影精度/`ENDGAME_RACE_WINDOW`/窗口 EV 好果下限/突破时间税/ETA 精度/任务·资源 race 阈值，并逐项打开 P3 开关验证；②实现 P4 §7 条件化 SET_GUARD（默认关，把 denial 胜负期望计入 ΔEV）。

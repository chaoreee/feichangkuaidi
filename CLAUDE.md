...
- 进度：**Phase 0✅（两批 30 局）+ Phase A 保真度✅ + Iter 25 投影/修复✅ + Phase B 杠杆确证✅ + Phase B v1/v2/v2+效率门 A/B 均未合入 + beat_top10 P0✅（Iter 29，设卡卡死修复无条件合入）+ P1-B✅（Iter 30，精简 trace 派生，纯观测）+ P1-A✅（Iter 31，分析器数据补全，纯观测）+ Iter 32✅（对手策略分类器 + 群体归因段，纯观测）+ Iter 32.5✅（codeagent 跑出 N=67 真实基线，64% 胜率/96% 交付）+ Iter 33✅（P0.2 MOVING_ACTION_FORBIDDEN 重试风暴修复，3 局未交付根因，无条件合入）+ Iter 34✅（冰鉴阈值 78→81 保鲜修复，弱优于无条件合入；§1「路线非杠杆」结论已勘误撤销——me 实际走山路绕行，路线 IS 杠杆）+ P2/P3 规划定稿未实现**。M8 P1-P4 代码全部保留作**工具箱**（race/guard flags 默认关），由 beat_top10 排期驱动逐项评估，不再单列。**前进计划严格按 `docs/iteration_loop_design.md`**（旧 M9 Phase D / calibration_v1 §8 已取代，见 §6）：~~Iter29 **P0** 修设卡卡死~~✅ → ~~Iter30 **P1-B** 精简 trace 回流通道~~✅ → ~~Iter31 **P1-A** 分析器数据补全~~✅ → ~~Iter32 **群体归因段** 对手分类器 + 对手类分桶~~✅ → ~~Iter32.5 codeagent 跑首轮真实基线（N=67）~~✅ → ~~Iter33 **P0.2** MOVING_ACTION_FORBIDDEN 重试风暴修复~~✅ → ~~Iter34 **冰鉴阈值保鲜**（78→81 弱优于合入；§1 路线证伪已勘误）~~✅ → **Iter35 山路绕行归因 + 路线鲜度感知修复**（`docs/iter35_plan.md`：现有 reports 归因 me 为何绕山→鲜度感知绕路门 / 重评 static_planner，stage 后 codeagent 真实 A/B N≥30）→ **Iter36+ 博弈最优**（对手策略分类器 + 对手类驱动策略切换）→ **Iter37+ P3 denial**。新"done"标准：**sim A/B 50 种子降为回归 + 不变量门**（0 STUCK/对账 0 误差/分段不回归；镜像自博弈无法验证博弈层）；**真实对战 A/B 升为合入门**（新 vs 老 client 对同一对手池 N≥30 正向才合入）；阈值合入须真实 trace N≥30；验证为负则删。策略须**通用**（读 `start` 动态决策，决赛换新图）。
...
| 错误码处理（协议 §11 全量） | ✅ | 立即 error 分类记录；拒绝反馈：PROCESS_REQUIRED 强制处理、移动阻塞类临时拉黑目标绕行(防循环)、CLAIM_TASK+OBJECT_BUSY task 级冷却（Iter 25）、**非 MOVE 动作+MOVING_ACTION_FORBIDDEN 按签名冷却**（Iter 33，防 WAITING 态连发 BREAK_GUARD/USE_RESOURCE/RUSH_PROTECT/FORCED_PASS 风暴） |
...
| 资源领取与使用（冰鉴保鲜/马加速/情报探路） | ✅ | 机会式领取+移动中用马；情报(INTEL)探路前方处理点/宫门减时(射程 15，含"可用性"领取守卫)；**Iter 34**：`ICE_BOX_USE_BELOW` 78→81——在跌破 80 好果转坏阈值前用冰鉴（任务书 §3.3.1 冰鉴 +10/转坏不可逆），端鲜度不变（=100−loss+10×ice 与时机无关）但 MIN 鲜度从 ~77 抬到 ~81、80 阈值不再触发 → 救回 1 篓好果（+1.8 分）。弱优于（中损耗区间 80≤F<90 严格省 1、其余相同）。**注**：Iter 34 §1「路线非杠杆」已勘误——me 实际走山路 S03-S06-S08 绕行领资源（多损 7.43 鲜度），路线 IS 杠杆，Iter 35 归因修复 |
...
| 移动/等待中主动续行（防卡死，保交付） | ✅ | `_keep_moving`：MOVING/WAITING 每帧重发 MOVE 到当前在途目标续行（协议允许、不清进度）；**Iter 29（P0）**：重发前校验在途目标是否失效（`_in_transit_target_blocked`：在节点冷却期 **或** 被对手设卡 owner!=我方），失效则丢弃在途目标回落 `_plan` 全量重规划（`_advance` 绕行 / `_breakthrough` FORCED_PASS/BREAK_GUARD）——修复 vs2735 真实败局（对手设卡封 S10、`MOVE_BLOCKED_BY_GUARD` 连拒 224 帧未交付；与 Iter 8 同源，补"在途目标失效"盲区）；无在途目标则重规划。己方设卡不挡己方。**Iter 33（P0.2）**：`_plan` 在 WAITING 态发的非 MOVE 节点动作（BREAK_GUARD/USE_RESOURCE/RUSH_PROTECT/FORCED_PASS）触 `MOVING_ACTION_FORBIDDEN` 连发成风暴烧光交付窗口（N=67 真实基线 3 局未交付根因：S10 WAITING 连发 ICE_BOX 201–287 帧/BREAK_GUARD 52 帧/RUSH_PROTECT 180 帧/FORCED_PASS 60 帧）——泛化 Iter 25/29 拒绝冷却：`_apply_rejection_feedback` 对非 MOVE+MOVING_ACTION_FORBIDDEN 按签名 `(action,target,resource)` 冷却（`REJECT_ACTION_COOLDOWN_ROUNDS=8`），`_freshness_rescue`/`_maybe_rush_protect`/`_maybe_bounty`/`_breakthrough` 加 `_action_cooled` 门，冷却中跳过→`_plan` 落到 `_advance`(MOVE) 重路由绕行；`_window_card` MOVING/WAITING 只出 ABSTAIN。`DELIVER`/`VERIFY_GATE` 不受该码影响不门控 |
...
**M10 真实日志驱动综合迭代优化（Iteration 29 起；原"打败前十名"战役，Iter 32 起泛化为平台综合优化）**：以"静态最优解 + 博弈最优解"为目标，对**平台真实对手群体**（代表性采样，前十名为首批高价值样本**但不限于**）持续采样、群体归因、A/B 验证、迭代。首批证据：10 局对前十名真实报告（4W/6L、我方固定点 755、胜负由对手鲜度决定、1 局被设卡卡死未交付）；**Iter 32.5 codeagent 真实基线 N=67**（64% 胜率/96% 交付，群体归因：quality-route 桶 W=0.43 鲜度劣势、3 局未交付风暴）。按 P0–P3 推进：**P0**✅ 修设卡卡死 bug（`_keep_moving` 在途目标失效回落 `_plan`，无条件合入）→ **P1-B**✅ 精简 trace 派生（`analysis/compact.py`，落 `reports/` 使 trace 可回流）→ **P1-A**✅ 分析器数据补全（client 富化 + parser 抽对手分项/设卡/资源/轨迹 + aggregator 对手分项段）→ **P0.2**✅（Iter 33）修 MOVING_ACTION_FORBIDDEN 重试风暴（3 局未交付根因，非 MOVE 动作按签名冷却，无条件合入）→ **P2.0**✅（Iter 34）冰鉴阈值保鲜（`ICE_BOX_USE_BELOW` 78→81 弱优于合入，救 1 好果 +1.8 分；§1「路线非杠杆」结论已勘误撤销——me 实际走山路绕行）→ **P2**（Iter 35）山路绕行归因 + 路线鲜度感知修复（`docs/iter35_plan.md`：归因 me 为何绕山领资源 → 鲜度感知绕路门 / 重评 static_planner，stage 后 codeagent 真实 A/B N≥30）→ **P3** denial（设卡/破卡/争抢，真实 trace A/B）。**〔Iter 32 起框架升级〕** codeagent 自动对战闭环取代手动收割；目标"静态最优解基础上追求博弈最优解"——先静态最优（不读对手抬地板），再博弈最优（对手策略分类器 + 对手类驱动策略切换，替代 mode 拨已封顶 task 绕路的无效杠杆）。**验证门重定义**：sim A/B 降为回归 + 不变量门（镜像自博弈无法验证博弈层），真实对战 A/B 升为合入门；flag 默认关、阈值合入须 N≥30、验证为负则删（不再累积 variant 平台）。详见 `docs/iteration_loop_design.md` §0.5。
...
| 轮次 | 日期 | 触发 | 能力增量 | 详情 |
|---|---|---|---|---|
| Iter 34 | 2026-07-05 | quality-route 桶鲜度 gap（N=30, W=0.43, opp 鲜度 93.2 vs me 82.6）→ roadmap 排静态规划器路线重选 | **路线杠杆证伪 + 冰鉴阈值保鲜修复（弱优于无条件合入）**：真实 Map 拓扑 Dijkstra 三准则（时间/鲜度/分数）最优路完全重合（WATER 边同时最快最保鲜）→ 路线选择非鲜度杠杆、静态规划器是错误工具不开启；定位根因为冰鉴时机（同路同天气 me loss 26.64 vs opp 13.79，端鲜度时机无关、gap 在 MIN 鲜度：me 77.26 跌 80 后用、opp 85.28 跌 80 前用）；`ICE_BOX_USE_BELOW` 78→81 在跌 80 前用冰鉴救回 1 好果（+1.8 分），任务书规则下弱优于证明（中损耗区间严格省 1、其余区间相同）。CLIENT_VERSION iter33→iter34。2 项新单测；393 全过；sim 50 种子零回归（no-op，收益仅真实图 F≥80 显现） | CHANGELOG §[34] |
| Iter 33 | 2026-07-05 | 真实基线 N=67 群体归因：3 局未交付（4%）为最大失分点，compact trace 同根因（WAITING 态连发非 MOVE 动作 MOVING_ACTION_FORBIDDEN 风暴） | **P0.2 MOVING_ACTION_FORBIDDEN 重试风暴修复（无条件合入，零策略风险）**：泛化 Iter 25/29 拒绝冷却——`_apply_rejection_feedback` 对非 MOVE 动作+MOVING_ACTION_FORBIDDEN 按签名 `(action,target,resource)` 冷却（`REJECT_ACTION_COOLDOWN_ROUNDS=8`，USE_RESOURCE 区分 ICE_BOX/HORSE）；`_freshness_rescue`/`_maybe_rush_protect`/`_maybe_bounty`/`_breakthrough` 加 `_action_cooled` 门，冷却中跳过→`_plan` 落到 `_advance`(MOVE) 重路由绕行；`_window_card` MOVING/WAITING 只出 ABSTAIN。DELIVER/VERIFY_GATE 不门控。CLIENT_VERSION iter31→iter33。10 项新单测；391 全过；sim 50 种子零回归（1.000 交付/0 STUCK/对账 0）。预期转化 3 局未交付→交付 | CHANGELOG §[33] |
| Iter 32 | 2026-07-05 | codeagent 框架升级（§0.5：自动对战闭环取代手动收割，群体归因导向） | **对手策略分类器 + 群体归因段（纯观测，零策略风险）**：新模块 `analysis/opponent_classifier.py` `classify_opponent` 三类 guard-type/quality-route/speed-route（优先级 guard>quality>speed，物理阈值不扫参，旧 trace 降级 unknown）+ `annotate_opp_class`；aggregator `build_analysis_report` 加「## 对手类分桶」段（每类 N/胜率/均分/鲜度·好果 me-opp gap/交付帧 + 跨类归因一句话）+ `build_index` 加 `opponentClass`；`__main__` 注入 `classification.opponentClass`+`oppClassSignals`。client 零改动、不 bump CLIENT_VERSION。14 项新单测；366 全过；sim 200 局回灌分桶段出现、对账 0 误差 | CHANGELOG §[32] |

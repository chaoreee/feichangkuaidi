...
- 最后更新：2026-07-03
- 当前轮次：Iteration 15（M8 博弈投影层 P2 窗口 EV §5.4：`decision.py` `_window_card` 重构为代价感知+档位门控——无代价牌(兵争/验牒/免费强行)按克制强度恒出，献贡(烧好果)仅非 CONSERVATIVE 且窗口价值明显+好果高于档位下限+鲜度≥80 时出，不为窗口烧马。config 增窗口 EV 常量。14 项新单测(共 170)全通过；mock 无窗口故零回归 @r48。**P2 至此仅剩 §5.1 行3 突破烧好果意愿未做**）
- 上一轮次：Iteration 14（M8 P2 终局交付 race §5.3：`_endgame_race_state`+race-aware `_rush_speed_warranted`——落后抢帧/领先护果；11 项新单测(共 156)）。更早各轮见 §7 迭代日志。
- P2 进度：§5.1 行1/2/4/5 ✅、§5.2 悬赏 ✅、§5.3 终局 race ✅、§5.4 窗口 EV ✅；**仅剩 §5.1 行3 突破烧好果意愿**。所有增量动作均过 `_can_afford`+（相关处）ΔEV 地板，信息不足默认 EVEN=既有基线。
...
| P2 其余（§5.1 行3 突破烧好果意愿） | ❌ | 唯一待做的 P2 子项：CONSERVATIVE 优先绕行/强制通行、AGGRESSIVE 更积极攻坚(每次过 ΔEV 地板)。触碰交付关键的 `_breakthrough` 路径，需真实 trace 验证后再动 |
...
- `world.opponent` 已进入投影总线（Layer 1，`strategy/projection.py`）并驱动 **P2 档位调参**（§5.1 行1/2/4）：`decision.py` 每帧按 mode 调节任务绕路目标/上限与护果令阈值，绕路动作过 §3.3 ΔEV 地板。但 mode 前中段因 confidence<0.55 恒为 EVEN（=既有默认），故实战差异主要出现在中后段切档时——须真实 trace 校准 `LEAD_SAFE`/confidence 后才会频繁生效。§5.2 悬赏机会主义（`_maybe_bounty`）、§5.3 终局交付 race（`_endgame_race_state`+race-aware `_rush_speed_warranted`：落后抢帧/领先护果）、§5.4 窗口 EV（`_window_card` 代价感知+档位门控）已接入。仅剩 §5.1 行3（突破烧好果意愿）与 Layer 3-4 未实现。窗口 EV 未做对手出牌倾向预测（需可靠历史+真实 trace）。
...
**M8 博弈投影层（P1+P1.5+P2 档位调参已实现，P2 其余与 P3-P4 待续）**：把 `world.opponent` 升级为策略一等输入，用"对手投影驱动的风险档位切换 + 分数质量地板"提升胜率与得分上限。落地顺序（详见 `docs/game_theory_projection_strategy.md` §10）：P0 真实 trace 败局归因（待真实 `logs/`）→ **✅ P1 投影总线**（纯观测）→ **✅ P1.5 分数质量地板** `net_score_delta` → **🟢 P2 低风险增量基本完成**（✅ §5.1 行1/2/4/5 档位调参+ΔEV 地板、✅ §5.2 悬赏机会主义、✅ §5.3 终局交付 race、✅ §5.4 窗口 EV；仅剩 §5.1 行3 突破烧好果意愿）→ P3 中风险 race（ETA/任务/鲜度/资源，逐项开关）→ P4 条件化 SET_GUARD（ROI 最低、最后做）。铁律：所有增量动作须同时过 `_can_afford`（时间）与 `ΔEV≥0`（分数）；信息不足默认 `EVEN` 保持基线。**下一步**：①拿真实对局 trace 归因（P0）+ 校准 `LEAD_SAFE`/confidence/投影精度/`ENDGAME_RACE_WINDOW`/窗口 EV 好果下限；②评估是否进入 P3（中风险 race，逐项开关+真实 trace）。

...
# Iter 36 §2 合入：§1 马感知 walker 在 sim/孤岛条件下确认大路双冰鉴 +20 杠杆（rules.py 严格）。
# **Iter 36 §3 真实 A/B 结果：无定论（INCONCLUSIVE），flag 保持开**（见 reports/iter36_ab_out/section3_verdict.md）：
#   平台 N_old=67 iter31 vs N_new=40 iter36，iter36 表观劣化（胜率 0.64→0.42、交付 0.96→0.85、
#   均分 729→649），但**经路线/资源下钻证伪了"大路致负"假设**：
#   ① static_planner 在真实对局里是**行为 no-op**——0/40 局走 canonical 大路 S03→S07（iter31 亦 0/67），
#      两版都走 S06→S08→S10 山路骨架；冰鉴领取数相同（~1，ICE_KEEP=3 未生效）；S10 到达帧相同（267 vs 268）、
#      交付帧相同（409 vs 406）。plan_route 在静态图选大路（§1.3），但实战有资源/任务 waypoint 时不选大路
#      （或被 ΔEV/效率门否决）→ +20 双冰鉴机制从未激活，鲜度收益 0 是 no-op 的直接表现，非"穿 S10 设卡杀区"。
#   ② 表观回归主要来自对手池构成混杂：iter31 把弱对手 2769 刷了 11 次（全胜）、iter36 仅匹配 2 次。
#      去掉 2769 偏斜后，27 共同对手 head-to-head 胜率（57% vs 40%, p=0.17）与交付率（94% vs 87%, p=0.29）
#      差异均在噪声内，不显著。
#   判决：非确凿负向，不回退。static_planner 既不兑现 +20（no-op）也未被证有害——保留 flag-on 待
#   guard-aware/waypoint 修复使大路真正被选中后再验。Iter 33(风暴修复)+Iter 34(冰鉴阈值 81) 同包未单独验。

...
- 进度：**Phase 0✅ + Phase A 保真度✅ + Iter 25 投影/修复✅**（ΔEV 地板输入首次可信）。M8 P1-P4 代码全部保留但 triage：Layer1/ΔEV/ETA 保留（ΔEV 输入现已可信）、P2/P3 race 待仿真 A/B 后逐项开关、P4 SET_GUARD **冻结**。**下一步**（按 `docs/p0_attribution.md` §6）：① 继续收割真实 trace 至 N≥30（含对手分项分→确认真实杠杆：好果/路线 vs 交付时机）；② 全量 static_planner 替换 `_plan` 瀑布（杠杆确认后）；③ D1 GATE race（真实证据）；④ Phase C 仿真驱动阈值/开关校准（产出 `calibration_v1.md`）。新"done"标准：任何改动须过仿真 A/B 证据（见 `iteration_plan_v2.md` §1.2）。

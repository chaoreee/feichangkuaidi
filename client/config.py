...
# Iter 36 §2 合入：§1 马感知 walker 确认大路双冰鉴 +20 杠杆（rules.py 严格），§1.3 证 plan_route 真实图
# 选大路；sim 50 种子回归门全过（+30 对称增益 / 1.000 交付 / 0 STUCK / 0 对账 / TASK_90 1.00 / 分段不回归）。
# 真实 A/B N≥30（codeagent，同时验 Iter 33+34+36）是合入门——若为负则回退此 flag（§0.5 纪律：验证为负则删）。
# 风险：大路多 ~30 帧，_can_afford 时间守卫须兜底回落山路（决策层 plan_route 已含 ΔEV 改道门 + 效率门）。

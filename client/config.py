...
# ---- 策略调参（M4 收益策略）----
ICE_BOX_USE_BELOW = 90.0            # 鲜度低于此且持有冰鉴时使用：在跌破 80 好果转坏阈值前补鲜度
# Iter 34：78→81（在跌破 80 前用，救 1 篓好果）。Iter 39：81→90（弱优于最优点）。
# 冰鉴 +10 一次性、封顶 100（任务书 §3.3.1，好果转坏不可逆）。端鲜度 = 110−total_loss，
# 与用冰时机无关（Iter 34 已证）。比较阈值 θ：
#   - 端鲜度：θ=81 与 θ=90 相同（110−loss，时机无关）。
#   - 用冰数：相同（路线足够长，两阈值都触发 1 次）。
#   - 逐帧鲜度：θ=90 在 [t90,t81] 区间 post-ice（99.9→90.9），θ=81 同区间 pre-ice（89.9→80.9）；
#     t81 后两版轨迹收敛（fresh90 ≡ fresh81）。故 fresh90 ≥ fresh81 逐帧成立、[t90,t81] 严格大。
#   - 封顶浪费：θ=90 用冰时 fresh=89.9→+10=99.9（零浪费）；θ=91 才开始浪费（90.9→100.9 截 100）。
#     故 90 是无浪费的最大阈值 = 最优点。
# 结论：θ=90 弱优于 θ=81——单调衰减下端鲜度/用冰数/好果数完全相同，仅在天气峰值（非单调）下严格
# 更安全（pre-ice 鲜度 89.9 vs 80.9，离 80 转坏阈值远 9 分）。真实证据：67/67 局 weather_hit、
# iter31 trace me 好果 174（3 篓转坏，冰鉴在 fresh 77.25 用）。零路线/交付风险。详见 docs/iter34_route_lever_analysis.md。
...
ICE_BOX_RACE_USE_BELOW = 92.0      # 鲜度劣势时提前用冰鉴的阈值（须 > ICE_BOX_USE_BELOW 才有意义）
# Iter 39：88→92。Iter 34 时 baseline 81、race 88 = 提前用；Iter 39 baseline 抬到 90（弱优于最优点）后，
# race 阈值须 >90 才仍"提前"。92：劣势时 fresh 91.x→+10 截 100（微浪费 1.x，但劣势局优先护 80 转坏阈值
# 而非惜封顶浪费）。race 默认关、未验证，仅保持功能不退化。
...
# 默认关：作 variant 平台。合入须真实对战 A/B N≥30 正向且分段不回归（§0.5 合入门）。
# **纪律（Iter 38 成文）**：sim 镜像自博弈 + 静态图离线投影（route_planner_eval 等）均**不得作策略合入门
#   或正向收益证据**——sim 仅回归/不变量门（0 STUCK / 0 对账 / 不卡死 / 物理一致 / 分段不回归），
#   静态图投影仅假设生成；二者结论须真实对局复验方可驱动策略。
# **Iter 36 §2 曾开 flag**：依据是 §1 马感知 walker 静态图投影"大路双冰鉴 +20"+ sim 50 种子回归门
#   "全过（+30 对称增益）"。**Iter 36 §3 真实 A/B 证伪**（见 reports/iter36_ab_out/section3_verdict.md）：
#   平台 N_old=67 iter31 vs N_new=40 iter36，下钻发现 static_planner 在真实对局里是**行为 no-op**——
#   0/40 局走 canonical 大路 S03→S07（iter31 亦 0/67），两版都走 S06→S08→S10 山路骨架；冰鉴领取数相同
#   （~1，ICE_KEEP=3 未生效）；S10 到达帧相同（267/268）、交付帧相同（409/406）。§1.3"plan_route 真实图
#   选大路"是**静态图结论**，实战有资源/任务 waypoint 时不选大路（被 ΔEV/效率门否决或候选挤出）→
#   +20 双冰鉴机制从未激活。sim +30 是镜像自博弈对称 artifact（两侧都走大路→对称），非收益证据。
# **Iter 38 判决：flag 回退为关**。合入依据含 sim/静态投影影响且实战 no-op、+20 未兑现——恢复干净基线。
#   代码保留：下一轮明确要重做"查 plan_route 实战不选大路根因"（instrument 决策日志查 waypoint 候选挤出
#   vs ΔEV/效率门否决），修通后由真实 A/B 正向再开。Iter 33(风暴修复)+Iter 34(冰鉴阈值 81) 同包未单独验。
ENABLE_STATIC_PLANNER = True
# **Iter 39：flag 重开（诊断模式）**。Iter 38 回退关后，离线复现 plan_route 选路：
#   裸图 / 注入真实 S08 task / HEAVY_RAIN 天气 三个口径下 plan_route 均**选大路**（+23，门通过），
#   但 Iter 36 §3 实战 0/40 选大路——差异在不可离线复现的实战状态（动态障碍 / 对手已领 S03/S07 冰源 /
#   平台图漂移），非马速建模（path_frames 不建模马但门仍通过）、非 task-ice 零和（两路均 task 封顶）。
#   故 Iter 39 不盲改路线逻辑，而是：①plan_route 加决策日志（debug 出参，纯观测不影响返回）落 trace，
#     实战路线切换时记 候选/门控/blocked——下一批真实 trace 即可定位实战不选大路的真因；
#   ②flag 重开：实战 no-op 时行为=baseline（Iter 36 已证，安全零回归），若实战状态偶不触发阻碍则大路
#     可能 fire 兑现 +20。合入门仍为真实 A/B N≥30 正向（Iter 38 纪律）；sim 仅回归门。
STATIC_PLANNER_ICE_USE_BELOW = 90.0   # 冰鉴使用阈值：对齐 L1 ICE_BOX_USE_BELOW=90（弱优于最优点，无封顶浪费）
...
STATIC_PLANNER_MIN_ROUTE_EFFICIENCY = 0.2  # 改道候选每帧效率下限 gain/extra_frames，仅对长绕路(extra≥15帧)生效。≈v2 乐观修正率（实际−3.7 vs 投影+7 / 60帧≈0.18/帧）：拒0.12低效长绕路、纳0.26任务长绕路。注：0.18 修正率源自 sim A/B（静态图镜像自博弈），按 Iter 38 纪律 sim 不作策略依据，此阈值仅作 variant 平台参数保留，flag 默认关不生效

...
- 最后更新：2026-07-04
- 当前轮次：Iteration 27（**Phase B v2 联合规划器 — 仿真 A/B 仍未过门槛，flag 保持关**）。针对 v1 评审结论（`project_route` 冻结 task_base、`plan_route` 候选集过窄、`_ice_detour_target` 分项式零和）落地真正联合求解：`project_route` 加 `_path_pickups`（沿途建模 task 领取[贪心到 130 封顶]+ice 收集[+停靠帧鲜度损耗]）→ `plan_route` 候选集加冰源/任务点 waypoint（含 ice+task 二段组合，读 `start` 拓扑动态生成、不写死节点=通用）→ 删 `_ice_detour_target`、flag-on 跳过 `_task_detour_target`。**Bug 修复**：初版 waypoint 拼接含回溯段致逐帧重规划振荡卡死（7/50 STUCK）→ `_via_path` 拒绝非简单路径，修复后 0 STUCK。**验收**：265 client 单测全过（新增 7 项多图自适应测试：冰源顺路图改道、冰源偏远图保直送）；sim A/B 50 种子 **仍未过门槛**——flag-on mean 744.1 vs baseline 747.8（−3.7）、交付 +60 帧。根因推进：投影系统性低估长绕路时间成本（未建模暴雨/山雾对水路/山路移动减速，且**未来天气对客户端隐藏**→乐观不可彻底消除）。`ENABLE_STATIC_PLANNER` **保持默认关**（CLIENT_VERSION 不 bump），代码保留作通用 variant 平台。**不强行逼 samples 正向**（过拟合初赛图违反通用原则）。详见 `docs/calibration_v1.md` §7。
- 上一轮次：Iteration 26（Phase B v1 分项式静态规划器，A/B 未过门槛）。更早各轮见 §7 迭代日志。
...
- 进度：**Phase 0✅（两批 30 局）+ Phase A 保真度✅ + Iter 25 投影/修复✅ + Phase B 杠杆确证✅ + Phase B v1 A/B❌ + Phase B v2 联合 A/B❌（均未合入）**。M8 P1-P4 代码全部保留但 triage：Layer1/ΔEV/ETA 保留、P2/P3 race 待仿真 A/B 后逐项开关、P4 SET_GUARD **冻结**。**下一步**（按 `docs/calibration_v1.md` §7）：① ΔEV 门加"每帧效率"维度（gain/extra_frames ≥ 阈值）吸收投影天气乐观（隐藏信息不可消除）——拒低效长绕路；② 接受 samples 中性、靠决赛新图（冰源顺路时）自然正向；③ 继续收割至 N≥30 脱"假设级"；④ D1 GATE race（保持 Phase B 后首位）；⑤ Phase C 阈值/开关校准。新"done"标准：任何改动须过仿真 A/B 证据（见 `iteration_plan_v2.md` §1.2）。策略须**通用**（读 `start` 动态决策，不针对初赛 samples 图适配，决赛换新图）。
...
| Phase B 联合静态规划器 v2（task+ice+route 一体投影，默认关） | 🟡(默认关，A/B 未过) | **Iter 27**（v1 评审后重做）：`strategy/static_planner.py` `project_route` 加 `_path_pickups`（沿途建模 task 领取[贪心到 130 封顶]+ice 收集[+停靠帧鲜度损耗]，路线选择首次能权衡 task-ice 零和）+ `plan_route` 候选集加冰源/任务点 waypoint（含 ice+task 二段组合，读 `start` 拓扑动态生成=通用）+ `_via_path` 拒绝非简单路径（防回溯振荡卡死）。删 v1 `_ice_detour_target`（分项式元凶）；flag-on 跳过 `_task_detour_target`。**sim A/B 50 种子仍未过门槛**：mean 744.1 vs 747.8（−3.7）、交付 +60 帧（投影低估长绕路时间成本：未建模暴雨/山雾减速，且未来天气隐藏→乐观不可消除）。多图单测证自适应（冰源顺路图改道、偏远图保直送）。`ENABLE_STATIC_PLANNER` 默认关，代码保留作通用 variant 平台。23 项单测。详见 `docs/calibration_v1.md` §7 |

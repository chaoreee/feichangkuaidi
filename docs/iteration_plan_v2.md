...
1. **sim = 回归/不变量门（非合入门）**：仿真器 50 种子 flag-on vs baseline 仅要求 0 STUCK、对账 0 误差、不卡死、物理一致、分段不回归。**不得要求 sim mean 终局分正向**——sim 是镜像自博弈（两侧同构→gap 恒 0→mode 恒 EVEN→对手条件型代码永不激活），对路线/冰鉴类抬地板改动双侧对称→结构性中性，sim mean 既不能背书也不能否决策略。〔Iter 38 成文：旧标准把"sim mean ≥ baseline"当合入门是 Iter 26–28 static_planner 被挡关 ~10 轮的制度源头；sim 越界作正向证据是 Iter 36 §2 误开 flag 的依据之一〕
2. **离线静态图投影仅作假设生成**：`analysis/route_planner_eval.py`/`route_weather_audit.py` 等 rules.py 严格投影可生成"大路 +20"类假设，但结论在实战 waypoint/资源/任务绕路下可能不成立（Iter 36 §1.3"plan_route 真实图选大路"被 §3 真实 A/B 证伪：0/40 选大路、no-op）→ **不得据此合入策略，须真实对局复验**。
3. **真实对战 A/B = 合入门**：新 client vs 老 client 对同一对手池 N≥30，胜率/均分/分段正向且不回归才合入。博弈层（P3、对手感知切换）只认此门。
4. 无 WAITING/MOVING 卡死回归（仿真器 + 真实 trace）。
5. 单帧决策仍在预算内（`config.DECISION_BUDGET`）。
6. 改动的每个阈值/开关在 `docs/calibration_v1.md`（或对应迭代文档）有证据行。
7. 涉及博弈层动作时：有真实 trace A/B 的胜率/分差证据（sim 镜像自博弈无法验证博弈层）。
8. （分析器自身改动）分析器单测全过 + `rules.py` 对账自检 0 误差。
9. **A/B 达最小样本**（N≥30 配对；罕见事件 N≥100），否则结论标"假设级"不合入。
10. **无分段回归**：variant 在当前已赢分段（如 `mid_lead`、`delivered`）上不显著降分/降胜率——保护成功路径，防单点劣化（§3.7/§3.8）。
11. **单局只作假设来源**：任何改动须基于全语料聚合统计 + 置信区间，不基于单局结论。
...
- **补静态前置**：M8 的 ΔEV 地板输入（投影分）原不可信（鲜度粗估 0.06 平摊）；**Iter 25 已升级为路线感知鲜度模型**（`freshness_loss_for_path`），ΔEV 输入现已可信。
...
`aggregator` 跨局聚合，输出**多份分工报告**（统一落仓库根 `reports/`，与规格文档 `docs/` 解耦）：

- **`reports/analysis_report.md`**（主报告，累积聚合）：跨局统计（交付率、交付帧分布、mean/median 分、各分项分分布、失败模式频次、task-90 达成率、投影误差分布、mode 切换频次）+ **场景分段视图**（见 §3.7）+ 异常局标记（见下）。
- **`reports/ab_report.md`**（A/B 配对对比，存在 variant 时产）：按 seed 配对 variant vs baseline 的胜率、mean 分差、配对胜负、95% 置信区间、交付率/各分段回归检查。
...
> **⚠️ Iter 24 真实 trace 证伪原"冲 90"主杠杆**（真实 TASK_90_REACH=1.00，90 必达）。Phase B 目标
> 已重定义为"交付时机 vs 质量积累"静态权衡，详见 `docs/p0_attribution.md` §4.1/§6。下文 §6.1/6.2/6.4
> 的"任务-90 可达性"论述为原设计存档，落地时按重定义目标实施。§6.3 鲜度投影升级已由 Iter 25 完成。
...
### 6.3 鲜度投影升级（顺带修 ΔEV 输入质量）— ✅ Iter 25 已完成

**Iter 25 落地**：`projection.freshness_loss_for_path` 按路线类型逐边累计（`FRESHNESS_LOSS_MOVE[rt]`×`frames_on_edge` + 处理站/验核停靠 `FRESHNESS_LOSS_BASE`×帧 ×天气系数），替换 `AVG_FRESHNESS_LOSS_PER_FRAME=0.06` 平摊；ΔEV 调用方（`_detour_net_delta`/绕路/deny/bounty）改逐边路线损耗差。ΔEV 地板与投影分首次可信。详见 CHANGELOG §[25]。
...
| ΔEV 地板 `net_score_delta` | **保留并升级输入** | **Iter 25 已升级路线感知鲜度模型，ΔEV 输入已可信** |
...
| Iter 24 ✅ | **Phase 0 归因 + 仿真器保真度校准** — 用户上传 11 局真实平台报告→`docs/p0_attribution.md`：证伪"冲 90"杠杆（真实 TASK_90_REACH=1.00）、暴露"早交付 vs 质量积累"真实静态权衡 + GATE race 真实证据；sim 任务池改 10 沿途任务+每玩家独立完成→TASK_90_REACH 0.04→1.00、交付帧 mean 436.7→455.4(≈真实 456)。N=11<30 标"假设级" | — |
| Iter 25 ✅ | **CLAIM_TASK 重试修复 + 鲜度投影升级** — `_apply_rejection_feedback` 对 CLAIM_TASK+OBJECT_BUSY 设 task 级冷却（防 S10 重发风暴）；`freshness_loss_for_path` 逐边路线感知替换 0.06 平摊，ΔEV 地板输入首次可信；sim 零回归，302 单测。全量 static_planner 推迟（task 已封顶，真实杠杆待对手分项 trace 确认） | A, 0 |
| Iter 26 | **Phase B（重定义，待对手分项 trace）**：静态规划器求解"交付时机 vs 质量积累"最优（非"冲 90"，90 必达） | A, 0 |
| Iter 27 | Phase C：阈值扫描 + 开关 A/B，产出 `calibration_v1.md` | A, B |
| Iter 28 | Phase D1：GATE 验核 race | A, C |
| Iter 29 | Phase D2：窗口 EV 对手出牌预测 | A, C |
| Iter 30 | Phase D3：deny 按 \|gap\| 条件化 + 相对分 ΔEV | A, C |

...
> 基于 2026-07-04 拉取的 10 局对战平台前十名真实报告（`reports/`，N=10 假设级）。
...
## 5. 建议立即执行
1. **P0 设卡卡死修复**（可本轮落地，单测 + sim A/B 验证 0 回归）。
2. **P1 分析器补全方案评审**：先核对 `docs/protocol.md` 中 inquire/over 对对手库存、任务、设卡的暴露程度，确定哪些字段可补、哪些协议层不可见（需 client 侧推断）。再由用户取新一轮 reports 回流。

...
# Iter 36 §2 曾合入：§1 马感知 walker 在 sim/孤岛条件下确认大路双冰鉴 +20 杠杆（rules.py 严格）。
# **Iter 36 §3 真实 A/B 判负 → 回退默认关**（见 reports/iter36_ab_out/section3_verdict.md）：
#   平台 N_old=67 iter31 vs N_new=40 iter36，iter36 全面劣化——
#   胜率 0.64→0.42、交付率 0.96→0.85（未交付 3→6）、均分 729→649、8/8 分段全回归 ⚠。
#   排除混杂：27 共同对手 head-to-head 仍 iter36 劣（交付 96%→88%、胜率 67%→41%、交付均分 762→754），
#   铁证 opp=2769(11W→1W)、2629(5W→0W)、2735(交付→未交付)。
#   根因：大路 S01→..→S10→S13→S14→S15 穿 S10/S14 设卡关隘，强对手(guard-type)封 S10 致
#   MOVE_BLOCKED_BY_GUARD+MOVING_ACTION_FORBIDDEN 风暴至 600 帧未交付（Iter29/33 修复被 planner
#   反复指回 S10 架空）；+20 鲜度收益在真实对抗中完全未兑现（交付局鲜度 148.1→147.4 持平、
#   task −7、总分 −8）——sim 自博弈无设卡故 +30 对称增益是假象。
#   §0.5 纪律：验证为负则回退 flag。代码保留作 variant（guard-aware 改造留待后验）。
#   Iter 33(风暴修复)+Iter 34(冰鉴阈值 81) 独立于路线、保留。

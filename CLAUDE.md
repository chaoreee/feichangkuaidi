...
- 最后更新：2026-07-04
- 当前轮次：Iteration 26（**Phase B v1 静态规划器 — 仿真 A/B 未过门槛，flag 保持关**）。基于 `docs/p0_attribution_batch2.md` 真实杠杆确证（鲜度 +19、质量路线投影 +24 上界）：落地 `strategy/static_planner.py`（`freshness_optimal_path` 鲜度加权 Dijkstra + `project_route` 终局分投影含冰鉴模型 + `plan_route` 候选路线×冰鉴用量取最高）+ `decision.py` `_ice_detour_target`（就近冰源绕路）+ `_select_path`/`_freshness_rescue`/`_maybe_claim` flag-gated 接入 + `sim_server --static-planner` flag。**验收**：258 client 单测全过；sim A/B 50 种子 **未过 §1.2 门槛**——冰鉴优先模式 mean +0.5（fresh 82→92、goodFruit 98→100 兑现，但 task 140→120 回归 −10、交付 +21 帧 −2），任务优先模式中性零回归。根因：分项式绕路下 task 与 ice 争同一 spare-time 预算（零和），+24 上界假设 task 不变被证伪。`ENABLE_STATIC_PLANNER` **保持默认关**（运行期行为零变化，CLIENT_VERSION 不 bump），代码保留作全量联合规划器的 variant 平台。详见 `docs/calibration_v1.md`。
- 上一轮次：Iteration 24（Phase 0 真实 trace 归因 + 仿真器保真度校准）。更早各轮见 §7 迭代日志。
...
- 进度：**Phase 0✅（两批 30 局）+ Phase A 保真度✅ + Iter 25 投影/修复✅ + Phase B 杠杆确证✅ + Phase B v1 A/B❌（未合入）**。M8 P1-P4 代码全部保留但 triage：Layer1/ΔEV/ETA 保留、P2/P3 race 待仿真 A/B 后逐项开关、P4 SET_GUARD **冻结**。**下一步**（按 `docs/calibration_v1.md` §6）：① **全量联合 static_planner**（真正的 Phase B：task bundle+ice+route 联合优化，解决 v1 的 task-ice 零和）；② **仿真器保真度**：补真实 trace 冰源/任务点位置校准 sim map（sim task_base 恒 140、冰源拓扑可能使 sim 对质量路线系统性悲观）；③ 继续收割至 N≥30 脱"假设级"；④ D1 GATE race（保持 Phase B 后首位）；⑤ Phase C 阈值/开关校准。新"done"标准：任何改动须过仿真 A/B 证据（见 `iteration_plan_v2.md` §1.2）。
...
| Phase B 静态规划器 v1（鲜度感知路线+冰鉴策略，默认关） | 🟡(默认关，A/B 未过) | **Iter 26**：`strategy/static_planner.py`（`freshness_optimal_path` 鲜度加权 Dijkstra + `project_route` 复用 rules.py 终局分投影含冰鉴模型[+10 鲜度/次、−1 帧、抵 1 转坏跨越] + `plan_route` 候选路线×冰鉴用量取投影分最高、改道门控 `STATIC_PLANNER_MIN_ROUTE_GAIN`）+ `decision.py` `_ice_detour_target`（就近冰源绕路，过 `_can_afford`+ΔEV）、`_select_path`（flag-on 用 plan_route）、`_freshness_rescue`（阈值 91 护 90 阈值带）、`_maybe_claim`（ice_keep=3）。**sim A/B 50 种子未过门槛**：mean +0.5、task 回归（task-ice 时间零和，+24 上界假设 task 不变被证伪）。`ENABLE_STATIC_PLANNER` 默认关，代码保留作全量联合规划器 variant 平台。16 项单测。详见 `docs/calibration_v1.md` |
